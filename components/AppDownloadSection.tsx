  ...
  useEffect(() => {
    const getReleases = async () => {
      const fRes = await fetch(
        "https://api.github.com/repos/bradleygichuru/budgetlite-appcast/releases",
        {
          headers: {
      ...
      console.log({ data });
      console.log({ url: data[0].assets[0]["browser_download_url"] });
      setRelease(data[0].assets[0]["browser_download_url"]);
    };
    getReleases();
    return () => {};
  }, []);
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="border-2 border-primary">
            <CardContent className="p-6 text-center">
              ...
              <Button
                className="w-full"
                onClick={() => window.open(release as string, "_blank")}
              >
                Download APK
              </Button>
            </CardContent>
